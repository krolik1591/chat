...
def my_promo_code_menu(sum_bets, balance_promo, ticket_promo=None, bonus_tickets=None):
    if sum_bets is None:
        sum_bets = 0

    if ticket_promo is None:
        text = one_active_promo_code_text(sum_bets, balance_promo)
    else:
        text = two_active_promo_code_text(sum_bets, balance_promo, ticket_promo, bonus_tickets)
