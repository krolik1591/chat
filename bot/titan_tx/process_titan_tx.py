...
async def process_titan_tx(user_id, username, ton_amount, context, token, user_withdraw_address):
    new_tx = await add_new_titan_tx(user_id=user_id, token_id=token.token_id, nano_ton_amount=ton_amount * 10**9,
    ...
    text, keyboard = process_titan_tx_menu(user_id, username, ton_amount, id_new_tx, token.token_id, token.price)
