...
async def create_fortune_wheel(request: Request):
    ...
    bonus = int(form_data['bonus'])
    type_promo = int(form_data['type'])
    number_of_uses = int(form_data['number_of_uses'])
    number_of_users = int(form_data['number_of_users'])
    time_of_existence = int(form_data['time_of_existence']) * 3600 * 24
    time_of_duration = int(form_data['time_of_duration']) * 3600 * 24
    min_wager = int(form_data['min_wager'])
    wager = int(form_data['wager'])
